...
python qqmusic_sales_task.py "https://c6.y.qq.com/base/fcgi-bin/u?__=fb1Etv2oaVhH" \
  --browser-probe --debug --min-value 10000 --probe-wait-ms 20000 --headed
```

> 建议本地先用 `--headed`（非无头）观察页面是否真的触发了接口请求。
...
- 脚本会过滤明显噪声字段（如 `count_ver`、comment/view/play、trace/version 等）。
- 当找不到结果时，`--debug` 现在会打印失败上下文，方便你直接定位到底有没有抓到接口响应。
